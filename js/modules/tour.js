...
const PASSOS = [
  {
    rota: "dashboard",
    titulo: "Dashboard",
    desc: "Aqui você tem a visão geral da operação: receita total, lucro líquido, valores a receber e todas as locações do período. É o ponto de partida do dia.",
  ...
  },
  {
    rota: "romaneio",
    titulo: "Romaneio",
    desc: "Tabela completa de todas as entregas: data, dia, tecnologia, cliente, endereço, horário, quantidade de horas e responsável — igual à sua planilha. O check-list de cada entrega fica aqui.",
  ...
  },
  {
    rota: "locacoes",
    titulo: "Locações",
    desc: "Cadastre e gerencie cada locação. O sistema calcula o lucro líquido automaticamente, diferencia frota própria de sublocada e controla o status de pagamento.",
  ...
  },
  {
    rota: "clientes",
    titulo: "Clientes",
    desc: "Base completa de clientes com endereço comercial, voltagem do local, horário de funcionamento e restrições de acesso. O endereço é preenchido automaticamente nas locações.",
  ...
  },
  {
    rota: "equipamentos",
    titulo: "Equipamentos",
    desc: "Cadastro de toda a frota com número de série, código QR, tecnologia e acessórios inclusos. Indica visualmente se o equipamento é próprio ou sublocado de parceiro.",
  ...
  },
  {
    rota: "motoristas",
    titulo: "Motoristas",
    desc: "Gerencie a equipe de entregas, tipos de vínculo e os PINs de acesso ao app do motorista. Gere novos PINs e copie a mensagem pronta para enviar pelo WhatsApp.",
  ...
  },
  {
    rota: "fornecedores",
    titulo: "Fornecedores",
    desc: "Cadastro de parceiros e sublocadores de equipamentos externos. Locações com equipamentos de parceiros aparecem sinalizadas visualmente no romaneio.",
  ...
  },
  {
    rota: "financeiro",
    titulo: "Financeiro",
    desc: "Controle completo de receitas, despesas fixas e avulsas, margem de lucro por locação e inadimplência. Registre pagamentos e acompanhe o fluxo do negócio.",
...
let passoAtual = 0;
let navegar = null;  // injetado pelo app.js
...
function injetarEstilos(){
  ...
  s.textContent = `
    ...
    #tour-bv .bv-card p{ font-size:12.5px; color:#64748b; line-height:1.5; }
    #tour-bv .bv-footer{ display:flex; gap:12px; justify-content:center; flex-wrap:wrap; }

    /* Balão do tour guiado */
    #tour-balao{
    ...
    #tour-balao .tb-ico{
      ...
      display:flex; align-items:center; justify-content:center; color:#0d4f8b;
      margin-bottom:12px;
    }
    #tour-balao .tb-prog{
    ...
    #tour-balao p{ font-size:13.5px; color:#475569; line-height:1.6; margin-bottom:18px; }
    #tour-balao .tb-btns{ display:flex; gap:8px; }
    #tour-balao .tb-btns button{ flex:1; padding:10px; border-radius:10px; font-weight:700; font-size:13.5px; border:none; cursor:pointer; }
    ...
    #tour-backdrop{
      position:fixed; inset:0; z-index:200; background:rgba(10,20,40,.6);
    }
  `;
...
export function abrirBoasVindas(navegarFn){
  ...
  overlay.innerHTML = `
    <div id="tour-bv">
      <div class="bv-head">
        ...
        <p>Sistema de gestão de locações de tecnologia estética · Conheça todos os módulos abaixo</p>
      </div>
      <div class="bv-body">
        <div class="bv-grid">
          ${PASSOS.map((p,i)=>`
        ...
        <div class="bv-footer">
          ...
          <button class="btn btn-primary" id="tour-comecar" style="min-width:180px">▶ Iniciar tour guiado</button>
        </div>
      </div>
    </div>`;
  ...
  // Clicar num card navega para o módulo e fecha
...
function iniciarTour(navegarFn){
  ...
  passoAtual = 0;
  injetarEstilos();
  // Garante que a sidebar está visível (remove collapsed)
...
function mostrarPasso(){
  ...
  }, 350); // aguarda o módulo renderizar
}
...
function posicionarBalao(navItem, p){
  ...
  // Posiciona abaixo do item destacado
  ...
  if(bRect.bottom > window.innerHeight - 10){
    ...
    balao.querySelector("::before"); // reset seta (simplificado)
  }
}

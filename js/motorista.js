/* ===================================================================
   MedConnect · App do Motorista (lógica)
   PIN → rotas com filtros → sino de notificações → checklist com
   upload de mídia da galeria + assinatura.
   =================================================================== */
import { Store } from "./modules/store.js";
import { motoristas } from "./modules/mock-data.js";
...
let motorista = null;      // motorista logado
let filtro = "hoje";
let dataEscolhida = "";
let entregas = [];         // entregas do motorista (tempo real)
let unsub = null;
...
function tentarLogin(){
  const m = motoristas.find(x=> x.pin===pinAtual);
  if(!m){
    $("#pin-error").textContent = "Código inválido. Tente novamente.";
    pinAtual = ""; $$(".pin-dot").forEach(d=>d.classList.remove("filled"));
    return;
  }
  entrar(m);
}
...
function iniciarEscuta(){
  ...
  unsub = Store.watch("locacoes", lista=>{
    entregas = lista.filter(l => (l.motorista||"").toLowerCase() === motorista.nome.toLowerCase());

    // 1) detecta mudanças e gera notificações
    const { unread } = reconciliar(motorista.nome, entregas);
    // 2) remove da lista as notificações já resolvidas e confirmadas
    limparResolvidas(motorista.nome, entregas);

    atualizarSino();
    renderRotas();
    // se o drawer estiver aberto, atualiza
...
function renderRotas(){
  const lista = aplicarFiltro(entregas).sort((a,b)=> (a.data+ (a.horario||"")).localeCompare(b.data+(b.horario||"")));
  const cont = $("#rotas");
  ...
  <div class="card ${l.checklistOk?"feito":""}">
    <div class="card-topo">
      ...
      <div class="card-info">
        ...
        ${l.responsavel?`<div class="linha"><span class="ic">👤</span><span class="txt"><b>Responsável pela locação</b>${esc(l.responsavel)}</span></div>`:""}
...
function renderDrawer(){
  ...
  if(!feed.length){
    lista.innerHTML = `<div class="vazio"><div class="big">🔔</div>
      <div style="font-weight:700;color:#334155">Sem novidades</div>
      <div style="margin-top:6px;font-size:14px">Você será avisado quando uma entrega for adicionada, alterada ou removida.</div></div>`;
    return;
  }
  const tag = { nova:"Nova", alterada:"Alterada", removida:"Removida" };
  lista.innerHTML = feed.map(n=>{
    const loc = entregas.find(x=>x.id===n.locId);
    const resolvida = (!loc || loc.checklistOk); // concluída ou já não está na rota
    const precisaConfirmar = resolvida && !n.confirmada;
    return `
      <div class="notif ${n.tipo} ${n.lida?"":"nao-lida"}">
        <div class="notif-top">
        ...
        ${precisaConfirmar
          ...
          : (resolvida ? `<div class="notif-badge-resolv">Aguardando confirmação...</div>`
                       : `<div class="notif-badge-resolv">Entrega ainda ativa na sua rota</div>`)}
      </div>`;
  }).join("");
...
function quando(iso){
  ...
  return mesmoDia ? `Hoje ${hora}` : `${dia} ${hora}`;
}

/* =================================================================
   CHECKLIST (tela cheia) + UPLOAD DE MÍDIA + ASSINATURA
   ================================================================= */
let midiaSelecionada = [];  // {file, url, tipo}
...
function abrirChecklist(loc){
    ...
    <div class="sec">
      ...
      ${itens.map((it,i)=>{
        ...
        return `<div class="item">
          <input type="checkbox" id="${id}" data-sec="${esc(sec)}" data-item="${esc(it)}" ${checked}>
          <label for="${id}">${esc(it)}</label></div>`;
      }).join("")}
    </div>`).join("");
  ...
  $("#checklist-tela").innerHTML = `
    ...
    <div class="tela-corpo">
      ...
      <div class="midia-area">
        ...
        ${MODO_SIMULADO ? `<div class="simulado-aviso">⚙️ Modo demonstração: as fotos aparecem aqui mas ainda não sobem ao Drive. Configure o Google Apps Script (veja o README) para ativar o upload real.</div>`:""}
      </div>
  ...
  $$("[data-fase]").forEach(b=> b.onclick=()=>{
    faseAtual=b.dataset.fase;
    $$("[data-fase]").forEach(x=>x.classList.toggle("active",x===b));
  });
...
function adicionarMidia(files){
  [...files].forEach(f=>{
    midiaSelecionada.push({ file:f, url:previewURL(f), tipo:f.type.startsWith("video")?"video":"foto", enviado:false });
  });
  renderMidia();
}
function renderMidia(){
  ...
  g.innerHTML = midiaSelecionada.map((m,i)=>`
    <div class="thumb">
      ${m.tipo==="video"
        ? `<video src="${m.url}" muted></video><span class="vid-ic">🎬</span>`
        : `<img src="${m.url}">`}
  ...
  $("#midia-status").textContent = midiaSelecionada.length
    ? `${midiaSelecionada.length} arquivo(s) selecionado(s)` : "";
}
...
async function salvarChecklist(loc){
  ...
  if(pendentes.length){
    ...
    const resultados = await enviarVarios(
      files,
      { locId:loc.id, cliente:loc.cliente, data:loc.data, fase:faseAtual },
      (i,total,nome)=>{ $("#midia-status").textContent = i<total?`Enviando ${i+1}/${total}...`:"Envio concluído"; });
